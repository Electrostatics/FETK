 ...
 * 
 * rcsid="$Id: mypde2.c,v 1.12 2010/08/12 05:17:26 fetk Exp $"
 * ***************************************************************************
 */
 ...
 *
 *     Problem type:          Doug Arnold's simple test problem
 *     Spatial dimension:     2
 *     Product dimension:     1
 *     Differential operator: General linear elliptic equation
 *     Boundary conditions:   General mixed Dirichlet/Robin conditions
 *
...
VEMBED(rcsid="$Id: mypde2.c,v 1.12 2010/08/12 05:17:26 fetk Exp $")
 ...
 *  
 *            du       du                      du       du
 *      ( a11 -- + a12 -- + b1 u ) nx  + ( a21 -- + a22 -- + b2 u ) ny
 *            dx       dy                      dx       dy
 *                                
 *           + k u + j = 0
 *  
...
VPRIVATE double u_coef(double x,double y) {
    double value;
    value = VEXP(-x*x-y*y);
    return value;
}

VPRIVATE double a11_coef(double x,double y) {
    double value;
    value = 1.0;
    return value;
}

VPRIVATE double a12_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double a21_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double a22_coef(double x,double y) {
    double value;
    value = 1.0;
    return value;
}

VPRIVATE double b1_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double b2_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double c1_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double c2_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double d_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double f_coef(double x,double y) {
    double value, u;
    u = u_coef(x,y);
    value = 4. * u * ( 1. - x*x - y*y );
    return value;
...
VPRIVATE double g_coef(double x,double y) {
    double value;
    value = u_coef(x,y);
    return value;
}

VPRIVATE double j_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}

VPRIVATE double k_coef(double x,double y) {
    double value;
    value = 0.0;
    return value;
}
    ...
    double V[], double dV[][3])
{
    ...
    if (key == 0) {
        ...
        value = -(a11_coef(x,y)*ux+a12_coef(x,y)*uy+b1_coef(x,y)*u)*vx
                -(a21_coef(x,y)*ux+a22_coef(x,y)*uy+b2_coef(x,y)*u)*vy
                +(c1_coef(x,y)*ux+c2_coef(x,y)*uy+d_coef(x,y)*u+f_coef(x,y))*v;
    ...
    double V[], double dV[][3])
{
    ...
    if (key == 0) {
        ...
        value = -(a11_coef(x,y)*vx+a12_coef(x,y)*vy+b1_coef(x,y)*v)*wx
                -(a21_coef(x,y)*vx+a22_coef(x,y)*vy+b2_coef(x,y)*v)*wy
                +(c1_coef(x,y)*vx+c2_coef(x,y)*vy+d_coef(x,y)*v)*w;
 ...
 * Routine:  rho
 *
 * Purpose:  At the single given point x, evaluate the mass density
 *           function (if you have one for your problem): rho = rho(x).
 *
 * Input:    PDE   = pointer to the PDE object
 *           chart = chart in which the point coordinates are provided
 *           txq[] = coordinates of the point
 ...
 * Output:   F[]   = resulting function values
 *
 * Speed:    This function is called by MC once for each quadrature point,
 *           so it should be fast.
 *
 * Author:   Michael Holst
 * ***************************************************************************
 */
VPUBLIC void rho(PDE *thee, int type,
    int chart, double txq[], double F[])
{
    F[0] = 1.;
}
...
VPUBLIC PDE* myPDE_ctor(void)
{
    ...
    thee->rho          = rho;           /* mass density function            */
