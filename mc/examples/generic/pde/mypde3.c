 ...
 * 
 * rcsid="$Id: mypde3.c,v 1.12 2010/08/12 05:17:26 fetk Exp $"
 * ***************************************************************************
 */
 ...
 *
 *     Problem type:          Karen Camarda's simple 2D system test problem #1
 *     Spatial dimension:     2
 *     Product dimension:     2
...
VEMBED(rcsid="$Id: mypde3.c,v 1.12 2010/08/12 05:17:26 fetk Exp $")
 ...
 * ***************************************************************************
 *
 * Two 2D, coupled linear equations with Dirichlet boundaries are solved. The
 ...
 *  dx   dx      dy   dy
 *
 * The bisectEdge appropriate for mesh3 (unit square) is used.
 *
 * ***************************************************************************
 */

VPRIVATE double f1_coeff(double x,double y)
{
    double value;
    value = VEXP(-x*x-y*y);
    return value;
...
VPRIVATE double f2_coeff(double x,double y)
{
    double value;
    value = VSIN(x*y);
    return value;
...
VPRIVATE double rho1_coeff(double x,double y)
{
    double value;
    value = (-2.+4.*x*x)*VEXP(-x*x-y*y) + (-2.+4.*y*y)*VEXP(-x*x-y*y) +
    VSIN(x*y);
    return value;
}

VPRIVATE double rho2_coeff(double x,double y)
{
    double value;
    value = -y*y*VSIN(x*y) - x*x*VSIN(x*y) - 0.5*VEXP(-x*x-y*y);
    return value;
}

VPRIVATE double a1_coeff(double x,double y)
{
    double value;
    value = 1.;
    return value;
...
VPRIVATE double a2_coeff(double x,double y)
{
    double value;
    value = -0.5;
    return value;
    ...
    double V[], double dV[][3])
{
    ...
    /* interior form case */
    /* Do I just sum the integrands for the two equations??? */
    if (key == 0) {
    
        value = -u1x*v1x-u1y*v1y+a1_coeff(x,y)*u2*v1-rho1_coeff(x,y)*v1
                -u2x*v2x-u2y*v2y+a2_coeff(x,y)*u1*v2-rho2_coeff(x,y)*v2;
    ...
    double V[], double dV[][3])
{
    ...
    if (key == 0) {
        ...
        value = -v1x*w1x-v1y*w1y+a1_coeff(x,y)*w2*v1
                -v2x*w2x-v2y*w2y+a2_coeff(x,y)*w1*v2;
...
VPUBLIC PDE* myPDE_ctor(void)
{
    ...
    thee->vec          = 2;             /* unknowns per spatial point;      */
