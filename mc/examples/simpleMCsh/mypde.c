 ...
 * 
 * rcsid="$Id: mypde.c,v 1.8 2010/08/12 05:17:46 fetk Exp $"
 * ***************************************************************************
 */
...
VEMBED(rcsid="$Id: mypde.c,v 1.8 2010/08/12 05:17:46 fetk Exp $")
...
VPRIVATE double my_USx0(int d, int m, double *x);
VPRIVATE double my_USx1(int d, int m, double *x);
VPRIVATE double my_USx2(int d, int m, double *x);
...
VPRIVATE double my_USx2(int d, int m, double *x) {
    return FACx2(d,m,x) * UU(d,m,x) + FAC(d,m,x) * UUx2(d,m,x);
}
...
VPRIVATE int sType, fType, fTypeComponent[MAXV];
    ...
    int chart, double tnvec[])
{
    ...
    for (i=0; i<thee->vec; i++) {
        fTypeComponent[i] = thee->bmap[i][faceType];
    }
}
...
{
    F[0] = my_US(thee->dim, thee->vec, txq);
    dF[0][0] = my_USx0(thee->dim, thee->vec, txq);
    dF[0][1] = my_USx1(thee->dim, thee->vec, txq);
    dF[0][2] = my_USx2(thee->dim, thee->vec, txq);
}
