 ...
 * 
 * rcsid="$Id: mypde2.c,v 1.3 2010/08/12 05:16:57 fetk Exp $"
 * ***************************************************************************
 */
 ...
 *
 *     Problem type:          2D linear elasticity (homogeneous,isotropic,
 *                                                  no volume load)
 *     Spatial dimension:     2
 *     Product dimension:     2
 *     Differential operator: Lame operator
 *     Boundary conditions:   mixed Dirichlet/Neumann BC (inhomogeneous OK)
 *
 * ***************************************************************************
 */
...
VEMBED(rcsid="$Id: mypde2.c,v 1.3 2010/08/12 05:16:57 fetk Exp $")
...
VPRIVATE double xq[3], nvec[3], vx[4][3], U[MAXV], dU[MAXV][3], g[3];
...
/*
 *   given a point, return the
 *   value of the exact solution there (return 0 if not known)
 */
VPRIVATE double u1_true(double x,double y) {
    double value;
    value = y;
    return value;
}
VPRIVATE double u2_true(double x,double y) {
    double value;
    value = 0.;
    return value;
}
...
/*
 *   given a point on the traction boundary and the face type (an even
 * integer) evaluate the value of the traction there and store in g[]
 */
VPRIVATE void evalTraction(double x,double y, int fType) {
    if (fType == 2) {
        g[0] = mu;
        g[1]=0.;
    } else if (fType == 4) {
...
/*
 *   given a point on the displacement boundary, return the
 *   value of the displacement there
 */
VPRIVATE double dirich1(double x,double y) {
    double value;
    value = u1_true(x,y);
    return value;
}
VPRIVATE double dirich2(double x,double y) {
    double value;
    value = u2_true(x,y);
    return value;
}
...
VPUBLIC void Fu(PDE *thee, int key, double F[])
{
    ...
    double x = xq[0];
    double y = xq[1];
    double s11, s12, s22, n1, n2;

    /* element residual case */
    /* since the operator is second order and there is no volume load, there is
       interior residual */
    if (key == 0) {
        for (i=0; i<thee->vec; i++)
            F[i] = 0.;
    ...
    /* neumann face residual case */
        ...
        n1 = nvec[0];
        n2 = nvec[1];
        F[0] = 2.*mu*(s11*n1 + s12*n2) + lambda*(s11+s22)*n1 - g[0];
        F[1] = 2.*mu*(s12*n1 + s22*n2) + lambda*(s11+s22)*n2 - g[1];
    /* interior face residual case */
        ...
        n1 = nvec[0];
        n2 = nvec[1];
        F[0] = 2.*mu*(s11*n1 + s12*n2) + lambda*(s11+s22)*n1;
        F[1] = 2.*mu*(s12*n1 + s22*n2) + lambda*(s11+s22)*n2;
    }
}
