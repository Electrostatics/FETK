/* ./src_f77/ngsdrvd.f -- translated by f2c (version 20030320).
...
/* * @brief   Driver for the nonlinear Gauss-Seidel iteration. */
/* * @version $Id: ngsdrvd.c,v 1.3 2010/08/12 05:45:25 fetk Exp $ */
/* * @attention */
/* * @verbatim */
/* * */
...
/* * @endverbatim */
/* * /////////////////////////////////////////////////////////////////////////// */
/* Subroutine */ int ngsrbdriv_(integer *iparm, doublereal *rparm, integer *
	...
	doublereal *ccf, doublereal *fcf, doublereal *tcf)
{
    ...
    /* Local variables */
    static integer n;
    extern /* Subroutine */ int ngsrbdriv2_(integer *, doublereal *, integer *
...
/* * purpose: */
/* * */
/* *    driver for a linear/nonlinear red/black gauss-seidel iteration. */
/* * */
/* * author:  michael holst */
/* * ********************************************************************* */
/* * */
...
/* *    *** some more checks on input *** */
	...
	s_wsle(&io___13);
	do_lio(&c__9, &c__1, "% NGSRBDRIV: real    work space must be: ", (
		ftnlen)41);
	do_lio(&c__3, &c__1, (char *)&iretot, (ftnlen)sizeof(integer));
	e_wsle();
	s_wsle(&io___14);
	do_lio(&c__9, &c__1, "% NGSRBDRIV: integer work space must be: ", (
		ftnlen)41);
...
/* *    *** call the multigrid driver *** */
    ngsrbdriv2_(&iparm[1], &rparm[1], &nx, &ny, &nz, &u[1], &iwork[k_iz__], &
...
/* Subroutine */ int ngsrbdriv2_(integer *iparm, doublereal *rparm, integer *
	...
	*fcf, doublereal *tcf)
{
    ...
    extern /* Subroutine */ int gsrbgo_(integer *, integer *, integer *, 
    ...
    extern /* Subroutine */ int nmatvec_(integer *, integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *), ngsrbgo_(integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     integer *, integer *, integer *, integer *, integer *, integer *,
	     doublereal *, doublereal *, doublereal *, integer *, doublereal *
	    , doublereal *, doublereal *, doublereal *, doublereal *);
...
/* *    *** stop timer *** */
    ...
    s_wsle(&io___44);
    do_lio(&c__9, &c__1, "% NGSRBDRIV2: fine problem setup time: ", (ftnlen)
	    39);
...
/* *    *** MATLAB *** */
    s_wsle(&io___47);
    do_lio(&c__9, &c__1, " gsrb = [ ", (ftnlen)10);
    e_wsle();
/* * */
/* *    *** start timer *** */
    tstart_(&bf, &oh);
...
/* *    *** call specified multigrid method *** */
    if (mode == 0 || mode == 2) {
	s_wsle(&io___48);
	do_lio(&c__9, &c__1, "% NGSRBDRIV2: linear mode...", (ftnlen)28);
	e_wsle();
	iok = 1;
	ilev = 1;
	gsrbgo_(nx, ny, nz, &u[1], &w0[1], &a1cf[1], &a2cf[1], &istop, &itmax,
		 &iters, &ierror, &iok, &iinfo, &epsiln, &errtol, &omegal, &
		ipc[1], &rpc[1], &ac[1], &cc[1], &fc[1], &tcf[1]);
    }
    if (mode == 1 || mode == 2) {
	s_wsle(&io___53);
	do_lio(&c__9, &c__1, "% NGSRBDRIV2: nonlinear mode...", (ftnlen)31);
	e_wsle();
	iok = 1;
	ilev = 1;
	ngsrbgo_(nx, ny, nz, &u[1], &w0[1], &a1cf[1], &a2cf[1], &istop, &
...
/* Subroutine */ int ngsrbgo_(integer *nx, integer *ny, integer *nz, 
	...
	doublereal *cc, doublereal *fc, doublereal *tru)
{
    ...
    extern /* Subroutine */ int ngsrb_(integer *, integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *, doublereal *, integer *
...
/* * purpose: */
/* * */
/* *    a nonlinear red/black gauss-seidel iteration. */
/* * */
/* * author:  michael holst */
/* * ********************************************************************* */
/* * */
...
/* *    *** do some i/o if requested *** */
    ...
    if (*iinfo != 0) {
	s_wsfe(&io___58);
	do_fio(&c__1, "% NGSRBGO: starting:", (ftnlen)20);
...
/* *       *** simply take norm of rhs for a zero initial guess *** */
	...
	s_wsle(&io___60);
	do_lio(&c__9, &c__1, "% NGSRBGO: bad istop value... ", (ftnlen)30);
	e_wsle();
    }
    if (rsden == 0.) {
	rsden = 1.;
	s_wsle(&io___61);
	do_lio(&c__9, &c__1, "% NGSRBGO:  rhs is zero ", (ftnlen)24);
	e_wsle();
    }
    rsnrm = rsden;
    orsnrm = rsnrm;
...
/* *       *** do 10 iterations *** */
    ...
    itmax_s__ = 10;
    omega_s__ = *omega;
    ngsrb_(nx, ny, nz, &ipc[1], &rpc[1], &ac[1], &cc[1], &fc[1], &x[1], &w1[1]
...
/* Subroutine */ int gsrbgo_(integer *nx, integer *ny, integer *nz, 
	doublereal *x, doublereal *r__, doublereal *w1, doublereal *w2, 
	integer *istop, integer *itmax, integer *iters, integer *ierror, 
	integer *iok, integer *iinfo, doublereal *epsiln, doublereal *errtol, 
	doublereal *omega, integer *ipc, doublereal *rpc, doublereal *ac, 
	doublereal *cc, doublereal *fc, doublereal *tru)
{
    ...
    static integer iadjoint;
    static doublereal errtol_s__;
    extern /* Subroutine */ int gsrb_(integer *, integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *, doublereal *, integer *
	    , integer *, doublereal *, doublereal *, integer *, integer *);
    extern doublereal xdot_(integer *, integer *, integer *, doublereal *, 
...
/* * purpose: */
/* * */
/* *    a linear red/black gauss-seidel iteration. */
/* * */
/* * author:  michael holst */
/* * ********************************************************************* */
/* * */
...
/* *    *** do some i/o if requested *** */
    ...
    if (*iinfo != 0) {
	s_wsfe(&io___71);
	do_fio(&c__1, "% GSRBGO: starting: ", (ftnlen)20);
	do_fio(&c__1, (char *)&(*nx), (ftnlen)sizeof(integer));
	do_fio(&c__1, (char *)&(*ny), (ftnlen)sizeof(integer));
	do_fio(&c__1, (char *)&(*nz), (ftnlen)sizeof(integer));
	e_wsfe();
...
/* *    *** compute denominator for stopping criterion *** */
	...
	s_wsle(&io___73);
	do_lio(&c__9, &c__1, "% GSRBGO: bad istop value... ", (ftnlen)29);
	e_wsle();
    }
    if (rsden == 0.) {
	rsden = 1.;
	s_wsle(&io___74);
	do_lio(&c__9, &c__1, "% GSRBGO:  rhs is zero ", (ftnlen)23);
	e_wsle();
    }
    rsnrm = rsden;
    orsnrm = rsnrm;
...
/* *       *** do 10 iterations *** */
    ...
    itmax_s__ = 10;
    omega_s__ = *omega;
    gsrb_(nx, ny, nz, &ipc[1], &rpc[1], &ac[1], &cc[1], &fc[1], &x[1], &w1[1],
