/* ./src_f77/sord.f -- translated by f2c (version 20030320).
...
/* * @brief   Successive Over-Relaxation iteration. */
/* * @version $Id: sord.c,v 1.3 2010/08/12 05:45:29 fetk Exp $ */
/* * @attention */
/* * @verbatim */
/* * */
...
/* * @endverbatim */
/* * /////////////////////////////////////////////////////////////////////////// */
/* Subroutine */ int sor_(integer *nx, integer *ny, integer *nz, integer *ipc,
	 ...
	 integer *iresid, integer *iadjoint)
{
    ...
    /* Local variables */
    extern /* Subroutine */ int sor7_(integer *, integer *, integer *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
	    doublereal *, doublereal *, integer *, integer *), sor27_(integer 
...
/* *    *** do in one step *** */
    ...
    numdia = ipc[11];
    if (numdia == 7) {
	sor7_(nx, ny, nz, &ipc[1], &rpc[1], &ac[ac_dim1 + 1], &cc[cc_offset], 
	...
	sor27_(nx, ny, nz, &ipc[1], &rpc[1], &ac[ac_dim1 + 1], &cc[cc_offset],
		 &fc[fc_offset], &ac[(ac_dim1 << 1) + 1], &ac[ac_dim1 * 3 + 1]
		, &ac[(ac_dim1 << 2) + 1], &ac[ac_dim1 * 5 + 1], &ac[ac_dim1 *
		 6 + 1], &ac[ac_dim1 * 7 + 1], &ac[(ac_dim1 << 3) + 1], &ac[
		ac_dim1 * 9 + 1], &ac[ac_dim1 * 10 + 1], &ac[ac_dim1 * 11 + 1]
		, &ac[ac_dim1 * 12 + 1], &ac[ac_dim1 * 13 + 1], &ac[ac_dim1 * 
		14 + 1], &x[x_offset], &w1[w1_offset], &w2[w2_offset], &r__[
		r_offset], itmax, iters, errtol, omega, iresid, iadjoint);
    } else {
	s_wsle(&io___2);
	do_lio(&c__9, &c__1, "% SOR: invalid stencil type given...", (ftnlen)
		36);
	e_wsle();
    }
/* * */
...
/* Subroutine */ int sor7_(integer *nx, integer *ny, integer *nz, integer *
...
/* * purpose: */
/* * */
/* *    fast 7 diagonal red/black sor routine. */
/* * */
...
/* mdir 3 3 */
		...
		for (i__ = ioff + 2; i__ <= i__4; i__ += 2) {
		    x[i__ + (j + k * x_dim2) * x_dim1] = x[i__ + (j + k * 
			    x_dim2) * x_dim1] * fac + *omega * (fc[i__ + (j + 
			    k * fc_dim2) * fc_dim1] + (on[i__ + (j + k * 
...
/* Subroutine */ int sor27_(integer *nx, integer *ny, integer *nz, integer *
	ipc, doublereal *rpc, doublereal *oc, doublereal *cc, doublereal *fc, 
...
/* mdir 3 3 */
		...
		for (i__ = 2; i__ <= i__4; ++i__) {
		    ...
		    x[i__ + (j + k * x_dim2) * x_dim1] = x[i__ + (j + k * 
			    x_dim2) * x_dim1] * fac + *omega * (fc[i__ + (j + 
			    k * fc_dim2) * fc_dim1] + (tmpo + tmpu + tmpd)) / 
