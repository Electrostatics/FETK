/* ./src_f77/zhgeqz.f -- translated by f2c (version 20030320).
...
/*     Machine Constants */
    ...
    anorm = zlanhs_("F", &in, &a[*ilo + *ilo * a_dim1], lda, &rwork[1], (
	    ftnlen)1);
    bnorm = zlanhs_("F", &in, &b[*ilo + *ilo * b_dim1], ldb, &rwork[1], (
	    ftnlen)1);
/* Computing MAX */
...
/*     Set Eigenvalues IHI+1:N */
	...
	if (absb > safmin) {
	    ...
	    if (ilschr) {
		i__2 = j - 1;
		zscal_(&i__2, &signbc, &b[j * b_dim1 + 1], &c__1);
		zscal_(&j, &signbc, &a[j * a_dim1 + 1], &c__1);
	    } else {
		i__2 = j + j * a_dim1;
		i__3 = j + j * a_dim1;
...
/*        Special case: j=ILAST */
	...
	if (ilast == *ilo) {
	    ...
	    if ((d__1 = a[i__2].r, abs(d__1)) + (d__2 = d_imag(&a[ilast + (
		    ilast - 1) * a_dim1]), abs(d__2)) <= atol) {
		i__2 = ilast + (ilast - 1) * a_dim1;
	...
	if (z_abs(&b[ilast + ilast * b_dim1]) <= btol) {
...
/*           Test 1: for A(j,j-1)=0 or j=ILO */
		...
		i__3 = j + (j - 1) * a_dim1;
		if ((d__1 = a[i__3].r, abs(d__1)) + (d__2 = d_imag(&a[j + (j 
			- 1) * a_dim1]), abs(d__2)) <= atol) {
		    i__3 = j + (j - 1) * a_dim1;
...
/*              Test 1a: Check for 2 consecutive small subdiagonals in A */
		...
		if (! ilazro) {
		    ...
		    i__4 = j + 1 + j * a_dim1;
		    i__5 = j + j * a_dim1;
		    if (((d__1 = a[i__3].r, abs(d__1)) + (d__2 = d_imag(&a[j 
			    + (j - 1) * a_dim1]), abs(d__2))) * (ascale * ((
			    d__3 = a[i__4].r, abs(d__3)) + (d__4 = d_imag(&a[
			    j + 1 + j * a_dim1]), abs(d__4)))) <= ((d__5 = a[
			    i__5].r, abs(d__5)) + (d__6 = d_imag(&a[j + j * 
			    a_dim1]), abs(d__6))) * (ascale * atol)) {
			ilazr2 = TRUE_;
		    }
		}
...
/*              this may have to be done repeatedly. */
		...
		if (ilazro || ilazr2) {
			...
			zlartg_(&ctemp, &a[jch + 1 + jch * a_dim1], &c__, &s, 
				&a[jch + jch * a_dim1]);
			i__4 = jch + 1 + jch * a_dim1;
			a[i__4].r = 0., a[i__4].i = 0.;
			i__4 = ilastm - jch;
			zrot_(&i__4, &a[jch + (jch + 1) * a_dim1], lda, &a[
				jch + 1 + (jch + 1) * a_dim1], lda, &c__, &s);
			i__4 = ilastm - jch;
			zrot_(&i__4, &b[jch + (jch + 1) * b_dim1], ldb, &b[
			...
			if ((d__1 = b[i__4].r, abs(d__1)) + (d__2 = d_imag(&b[
				jch + 1 + (jch + 1) * b_dim1]), abs(d__2)) >= 
				btol) {
			    if (jch + 1 >= ilast) {
				goto L60;
			    } else {
...
/*                 Then process as in the case B(ILAST,ILAST)=0 */
			...
			zlartg_(&ctemp, &b[jch + 1 + (jch + 1) * b_dim1], &
				c__, &s, &b[jch + (jch + 1) * b_dim1]);
			i__4 = jch + 1 + (jch + 1) * b_dim1;
			b[i__4].r = 0., b[i__4].i = 0.;
			if (jch < ilastm - 1) {
			    i__4 = ilastm - jch - 1;
			    zrot_(&i__4, &b[jch + (jch + 2) * b_dim1], ldb, &
			...
			i__4 = jch + 1 + jch * a_dim1;
			ctemp.r = a[i__4].r, ctemp.i = a[i__4].i;
			zlartg_(&ctemp, &a[jch + 1 + (jch - 1) * a_dim1], &
				c__, &s, &a[jch + 1 + jch * a_dim1]);
			i__4 = jch + 1 + (jch - 1) * a_dim1;
			a[i__4].r = 0., a[i__4].i = 0.;
			i__4 = jch + 1 - ifrstm;
			zrot_(&i__4, &a[ifrstm + jch * a_dim1], &c__1, &a[
				ifrstm + (jch - 1) * a_dim1], &c__1, &c__, &s)
				;
			i__4 = jch - ifrstm;
			zrot_(&i__4, &b[ifrstm + jch * b_dim1], &c__1, &b[
...
L50:
	...
	zlartg_(&ctemp, &a[ilast + (ilast - 1) * a_dim1], &c__, &s, &a[ilast 
		+ ilast * a_dim1]);
	i__2 = ilast + (ilast - 1) * a_dim1;
	a[i__2].r = 0., a[i__2].i = 0.;
	i__2 = ilast - ifrstm;
	zrot_(&i__2, &a[ifrstm + ilast * a_dim1], &c__1, &a[ifrstm + (ilast - 
		1) * a_dim1], &c__1, &c__, &s);
	i__2 = ilast - ifrstm;
	zrot_(&i__2, &b[ifrstm + ilast * b_dim1], &c__1, &b[ifrstm + (ilast - 
		1) * b_dim1], &c__1, &c__, &s);
	if (ilz) {
...
L60:
	absb = z_abs(&b[ilast + ilast * b_dim1]);
	if (absb > safmin) {
	    i__2 = ilast + ilast * b_dim1;
	    z__2.r = b[i__2].r / absb, z__2.i = b[i__2].i / absb;
	    d_cnjg(&z__1, &z__2);
	    ...
	    if (ilschr) {
		i__2 = ilast - ifrstm;
		zscal_(&i__2, &signbc, &b[ifrstm + ilast * b_dim1], &c__1);
		i__2 = ilast + 1 - ifrstm;
		zscal_(&i__2, &signbc, &a[ifrstm + ilast * a_dim1], &c__1);
	    } else {
		i__2 = ilast + ilast * a_dim1;
...
/*        Now check for two consecutive small subdiagonals. */
	...
	for (j = ilast - 1; j >= i__2; --j) {
	    ...
	    i__3 = j + (j - 1) * a_dim1;
	    if (((d__1 = a[i__3].r, abs(d__1)) + (d__2 = d_imag(&a[j + (j - 1)
		     * a_dim1]), abs(d__2))) * temp2 <= temp * atol) {
		goto L90;
	    }
/* L80: */
	}
...
/*        Sweep */
	...
	for (j = istart; j <= i__2; ++j) {
		...
		zlartg_(&ctemp, &a[j + 1 + (j - 1) * a_dim1], &c__, &s, &a[j 
			+ (j - 1) * a_dim1]);
		i__3 = j + 1 + (j - 1) * a_dim1;
		a[i__3].r = 0., a[i__3].i = 0.;
	    }
...
/* L110: */
	    ...
	    zlartg_(&ctemp, &b[j + 1 + j * b_dim1], &c__, &s, &b[j + 1 + (j + 
		    1) * b_dim1]);
	    i__3 = j + 1 + j * b_dim1;
	    b[i__3].r = 0., b[i__3].i = 0.;
