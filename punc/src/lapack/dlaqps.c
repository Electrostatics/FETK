/* ./src_f77/dlaqps.f -- translated by f2c (version 20030320).
...
/*        A(RK:M,K) := A(RK:M,K) - A(RK:M,1:K-1)*F(K,1:K-1)'. */
	...
	if (k > 1) {
	    i__1 = *m - rk + 1;
	    i__2 = k - 1;
	    dgemv_("No transpose", &i__1, &i__2, &c_b7, &a[rk + a_dim1], lda, 
...
/*                    *A(RK:M,K). */
	...
	if (k > 1) {
	    ...
	    i__1 = k - 1;
	    dgemv_("No transpose", n, &i__1, &c_b8, &f[f_dim1 + 1], ldf, &
...
/*        A(RK,K+1:N) := A(RK,K+1:N) - A(RK,1:K)*F(K+1:N,1:K)'. */
	...
	if (k < *n) {
	    i__1 = *n - k;
	    dgemv_("No transpose", &i__1, &k, &c_b7, &f[k + 1 + f_dim1], ldf, 
...
/* Computing MIN */
	...
	i__1 = *m - rk;
	i__2 = *n - *kb;
	dgemm_("No transpose", "Transpose", &i__1, &i__2, kb, &c_b7, &a[rk + 
