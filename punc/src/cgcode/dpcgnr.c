/* ./src_f77/dpcgnr.f -- translated by f2c (version 20030320).
...
/* Subroutine */ int dpcgnr_(S_fp matvec, S_fp pcondl, doublereal *a, integer 
	...
	doublereal *cndwk, integer *ierror)
{
    ...
    static char fmt_6[] = "(\002 THE METHOD IS CG ON CT*AT*A*C (PCGNR)\002,/)"
	    ;
    static char fmt_8[] = "(4x,\002CNDPNS = \002,d12.5,/)";
    ...
    static doublereal snorm, s0norm;
...
/*    DPCGNR IMPLEMENTS DCGNR ON THE PRECONDITIONED NORMAL EQUATIONS, */
...
/*    WHEN SNORM/S0NORM .LE. ERRTOL.  (ALSO SEE THE PROLOGUE TO DCGDRV.) */
...
/*    DPCGNR IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE: */

/*    DCGDRV: AN INTERFACE TO ANY ROUTINE IN THE PACKAGE. */
/*    DCG:    METHOD OF CONJUGATE GRADIENTS, A SPD. */
/*    DCR:    METHOD OF CONJUGATE RESIDUALS, A SYMMETRIC. */
/*    DCRIND: METHOD OF CONJUGATE RESIDUALS, A SYMMETRIC INDEFINITE. */
/*    DPCG:   PRECONDITIONED CG, BOTH A AND C SPD. */
/*    DCGNR:  CG APPLIED TO AT*A, A ARBITRARY. */
/*    DCGNE:  CG APPLIED TO A*AT, A ARBITRARY. */
/*    DPCGNR: CG APPLIED TO CT*AT*A*C, A AND C ARBITRARY. */
/*    DPCGNE: CG APPLIED TO C*A*AT*CT, A AND C ARBITRARY. */
/*    DPPCG:  POLYNOMIAL PRECONDITIONED CG, BOTH A AND C SPD. */
/*    DPCGCA: CG APPLIED TO C(A)*C*A, BOTH A AND C SPD. */
...
/* ***FIRST EXECUTABLE STATEMENT  DPCGNR */
...
/*     *** CHECK THE INPUT PARAMETERS *** */
	...
	s_wsfe(&io___10);
	do_fio(&c__1, (char *)&cndpns, (ftnlen)sizeof(doublereal));
	e_wsfe();
    }
    if (iounit > 0) {
...
/* L20: */
    ...
    s0norm = dnrm2_(n, &b[1], &c__1);
    if (iounit > 0) {
	io___21.ciunit = iounit;
	s_wsfe(&io___21);
	do_fio(&c__1, (char *)&s0norm, (ftnlen)sizeof(doublereal));
	e_wsfe();
    }

/*     *** CHECK THE INITIAL RESIDUAL *** */
    jstop = mdstop_(&istop, &iters, &itmax, &errtol, &stptst, ierror, &ap[1], 
