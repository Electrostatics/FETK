...
C            APPLYING THE METHOD OF CONJUGATE GRADIENTS TO THE NORMAL
C            EQUATIONS, AT*AX = AT*P, WHERE AT IS THE TRANSPOSE OF A.
C***DEDCRIPTION
C
C--- ON ENTRY ---
C
...
C***LONG DEDCRIPTION
C
C    DCGNR IMPLEMENTS THE CLASSICAL CONJUGATE GRADIENT METHOD APPLIED TO 
C    THE NORMAL EQUATIONS AT*A*X=AT*B, USING THE OMIN ALGORITHM:
C
C                   H0 = AT*R0
C                   P0 = H0
C                   ALPHA = <H,H>/<A*P,A*P>
...
C    MATHEMATICALLY, IF AT*A HAS M DISTINCT EIGENVALUES, THE ALGORITHM
C    WILL CONVERGE IN AT MOST M STEPS.  AT EACH STEP THE ALGORITHM
C    MINIMIZES THE 2-NORM OF THE RESIDUAL.  
C
C    ALTHOUGH THIS METHOD WILL CONVERGE FOR ANY A, IT MAY CONVERGE VERY
C    SLOWLY. THIS IS BECAUSE THE CONDITION NUMBER OF AT*A IS THE SQUARE
C    OF THAT OF A. IF A IS KNOWN TO HAVE ITS SPECTRUM IN THE RIGHT HALF
C    COMPLEX PLANE, THE USER SHOULD CONSIDER THE CHEBYSHEV ITERATION.
C
C    WHEN THE USER SELECTS THE STOPPING CRITERION OPTION ISTOP=0, THEN
C    THE CODE STOPS WHEN COND(AT*A)*(HNORM/H0NORM) .LE. ERRTOL, THEREBY
C    ATTEMPTING TO GUARANTEE THAT (FINAL RELATIVE ERROR) .LE. ERRTOL.
C    A NEW ESTIMATE FOR COND(AT*A) IS COMPUTED EVERY ICYCLE STEPS. THIS
C    IS DONE BY COMPUTING THE MIN AND MAX EIGENVALUES OF AN ORTHOGONAL 
C    SECTION OF AT*A.  THE LARGEST ORTHOG SECTION HAS ORDER ICYCLE*NCE,
...
C
C    IN THE IMPLEMENTATION BELOW THE VECTORS H AND AP SHARE SPACE.
C
C    DCGNR IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***ROUTINES CALLED  DONEST,D1MACH,DCGCHK,DAXPY,DNRM2
C***END PROLOGUE  DCGNR
C
C     *** DECLARATIONS ***
...
C
C***FIRST EXECUTABLE STATEMENT  DCGNR
 1    CONTINUE
C
C     *** INITIALIZE INPUT PARAMETERS ***
      ...
      KMAX   = ICYCLE*NCE
      ERRTOL = RPARAM(1)
      CNDATA = DMAX1(1.0D0, RPARAM(31))
C
C     *** CHECK THE INPUT PARAMETERS ***
      IF (IOUNIT .GT. 0) WRITE(IOUNIT,6)
 6    FORMAT(' THE METHOD IS CG ON AT*A (CGNR)', /)
      CALL DCGCHK(IPARAM,RPARAM,N)
      IF (IOUNIT .GT. 0) WRITE(IOUNIT,8) CNDATA
 8    FORMAT(4X, 'CNDATA = ', D12.5, /)
      IF (IOUNIT .GT. 0) WRITE(IOUNIT,10)
 10   FORMAT(' RESID  = 2-NORM OF AT*R', /,
     2       ' RELRSD = RESID / INITIAL RESID', /,
     3       ' COND(AT*A) USED IN STOPPING CRITERION', /)
C
...
C     *** TELL MDSTOP WHETHER OR NOT I AM SUPPLYING THE STOPPING QUANTITY ***
      IDO = 1
C
C     *** COMPUTE THE INITIAL H = AT*R ***
      CALL MATVEC(0,A,IA,WDUMMY,X,R,N)
      DO 20 I = 1, N
         R(I) = B(I) - R(I)
 20   CONTINUE
      CALL MATVEC(1,A,IA,WDUMMY,R,B,N)
      H0NORM = DNRM2(N,B,1)
...
C        *** UPDATE PARAMETERS ***
 ...
 70         FORMAT(/, ' NEW ESTIMATES FOR AT*A:')
            CALL DONEST(IOUNIT,D,E,CNDWK(1),CNDWK(KMAX+1),
     2                  IWORK,ITERS,EIGMIN,EIGMAX,CNDATA)
         ENDIF
      ENDIF
C
C     *** RESUME CGNR ITERATION ***
      GOTO 30
C
