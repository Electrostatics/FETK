      SUBROUTINE DPCGNE(MATVEC,PCONDL,A,IA,X,B,N,Q,IQ,IPARAM,RPARAM,
     2   IWORK,R,H,AP,D,E,CNDWK,IERROR)
C***BEGIN PROLOGUE  DPCGNE
...
C            NORMAL EQUATIONS.  SEE THE LONG DEDCRIPTION FOR DETAILS.
C***DEDCRIPTION
C
C--- ON ENTRY ---
C
...
C***LONG DEDCRIPTION
C
C    DPCGNE IMPLEMENTS DCGNE ON THE PRECONDITIONED NORMAL EQUATIONS,
...
C
C    DPCGNE IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***FIRST EXECUTABLE STATEMENT  DPCGNE
 1    CONTINUE
C
...
C        *** UPDATE PARAMETERS ***
 ...
 80         FORMAT(' NEW COND EST FOR C*A  = ', D12.5, /)
         ENDIF
      ENDIF
C
