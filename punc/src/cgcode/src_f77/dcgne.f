      SUBROUTINE DCGNE(MATVEC,A,IA,X,B,N,IPARAM,RPARAM,IWORK,
     2   R,H,D,E,CNDWK,IERROR)
C***BEGIN PROLOGUE  DCGNE
...
C            EQUATIONS, A*AT*Y = P, WHERE AT IS THE TRANSPOSE OF A AND
C            X = AT*Y. 
C***DEDCRIPTION
C
C--- ON ENTRY ---
...
C***LONG DEDCRIPTION
C
C    DCGNE IMPLEMENTS THE CLASSICAL CONJUGATE GRADIENT METHOD APPLIED TO 
C    THE NORMAL EQUATIONS A*AT*Y=B, X=A*Y, USING THE OMIN ALGORITHM:
C
C                   H0 = AT*R0
C                   P0 = H0
C                   ALPHA = <R,R>/<P,P>
...
C                   RNEW = R - ALPHA*(A*P)
C                   HNEW = AT*RNEW
C                   BETA = <RNEW,RNEW>/<R,R>
C                   PNEW = HNEW + BETA*P
C
C    THIS ALGORITHM IS GUARANTEED TO CONVERGE FOR ANY NONSINGULAR A.
C    MATHEMATICALLY, IF A*AT HAS M DISTINCT EIGENVALUES, THE ALGORITHM
...
C    SLOWLY. THIS IS BECAUSE THE CONDITION NUMBER OF A*AT IS THE SQUARE
...
C    SECTION OF A*AT.  THE LARGEST ORTHOG SECTION HAS ORDER ICYCLE*NCE,
...
C    WHEN RNORM/R0NORM .LE. ERRTOL.  (ALSO SEE THE PROLOGUE TO DCGDRV.)
C
...
C
C    DCGNE IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***FIRST EXECUTABLE STATEMENT  DCGNE
 1    CONTINUE
C
...
C     *** COMPUTE STOPPING CRITERION DENOMINATOR ***
      DENOM = 1.0D0
      IF (ISTOP .EQ. 0) DENOM = DNRM2(N,B,1)
      IF (ISTOP .EQ. 2) DENOM = DNRM2(N,B,1)
      IF (ISTOP .EQ. 4) DENOM = DNRM2(N,B,1)
C
...
C        *** UPDATE PARAMETERS ***
 ...
 70         FORMAT(/, ' NEW ESTIMATES FOR A*AT:')
            CALL DONEST(IOUNIT,D,E,CNDWK(1),CNDWK(KMAX+1),
     2                  IWORK,ITERS,EIGMIN,EIGMAX,CNDAAT)
            CONDA = DSQRT(CNDAAT)
            IF (IOUNIT .GT. 0) WRITE(IOUNIT,80) CONDA
 80         FORMAT(' NEW COND ESTIMATE FOR A  = ', D12.5, /)
         ENDIF
      ENDIF
C
C     *** RESUME CGNE ITERATION ***
      GOTO 30
C
