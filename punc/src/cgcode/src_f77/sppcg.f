      SUBROUTINE SPPCG(MATVEC,PCONDL,A,IA,X,B,N,Q,IQ,IPARAM,RPARAM,
     2   IWORK,R,H,W1,W2,W3,W4,D,E,IERROR)
C***BEGIN PROLOGUE  SPPCG
...
C
C    SPPCG IMPLEMENTS THE PRECONDITIONED CONJUGATE GRADIENT METHOD,
...
C
C    SPPCG IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***ROUTINES CALLED  SDPCHB,SCBFIX,SONEST,R1MACH,SAXPY,SCGCHK,SCKCHB,
C                    SDOT,SNRM2
C***END PROLOGUE  SPPCG
C
C     *** DECLARATIONS ***
...
C
C***FIRST EXECUTABLE STATEMENT  SPPCG
 1    CONTINUE
C
C     *** CHECK THE INPUT PARAMETERS ***
 ...
 5    FORMAT(' THE METHOD IS POLYNOMIAL PPCG', /)
      ...
      CALL SCKCHB(IPARAM,RPARAM,PEGMIN,PEGMAX,CONDCA)
      IF (IOUNIT .GT. 0) THEN
         IF (IPCOND .EQ. 0) WRITE(IOUNIT,10)
         IF (IPCOND .EQ. 1) WRITE(IOUNIT,11)
 10      FORMAT(' RESID  = SQRT(<C(A)R,R>)', /,
     2          ' RELRSD = RESID / INITIAL RESID', /)
 11      FORMAT(' RESID  = SQRT(<C(C*A)C*R,R>)', /,
     2          ' RELRSD = RESID / INITIAL RESID', /)
      END IF
C
...
C     *** COMPUTE STOPPING CRITERION DENOMINATOR ***
      ...
      IF ((ISTOP .EQ. 0) .OR. (ISTOP .EQ. 4)) THEN
         IF (IPCOND .EQ. 0) THEN
            CALL SCBFIX(MATVEC,PCONDL,A,IA,Q,IQ,IPCOND,
     2                  AA,BB,NCHB,B,H,W1,W2,W3,N)
            DENOM = SQRT(SDOT(N,B,1,H,1))
         ELSE
...
C        *** SAVE PARAMETERS ***
 ...
 40         FORMAT(/, ' NEW ESTIMATES FOR C(A)*A:')
            CALL SONEST(IOUNIT,D,E,W1,W2,IWORK,
     2                  KSTEP,PEGMIN,PEGMAX,CONDCA)
            EREPS = ERRTOL*ERRED
            CALL SDPCHB(IOUNIT,AA,BB,NCHB,PEGMIN,
