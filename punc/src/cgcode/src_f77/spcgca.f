      SUBROUTINE SPCGCA(MATVEC,PCONDL,A,IA,X,B,N,Q,IQ,IPARAM,RPARAM,
     2   IWORK,P,H,CAP,W1,W2,W3,D,E,IERROR)
C***BEGIN PROLOGUE  SPCGCA
...
C             OPTIMAL PRECONDITIONING POLYNOMIAL.
C
C    P,H,CAP  REAL(N), REAL(N), REAL(N).
...
C
C    SPCGCA IMPLEMENTS THE PRECONDITIONED CONJUGATE GRADIENT METHOD 
...
C
C    SPCGCA IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***FIRST EXECUTABLE STATEMENT  SPCGCA
 1    CONTINUE
C
...
C     *** COMPUTE NEW X AND H ***
      ALPHA = HNORM**2 / SDOT(N,CAP,1,P,1)
      DO 37 I = 1, N
         X(I) = X(I) + ALPHA*P(I)
         H(I) = H(I) - ALPHA*CAP(I)
 37   CONTINUE
...
C     *** CHECK NEW H ***
 ...
 40   FORMAT(' ITERS = ',I5,4X, 'RESID = ',E12.5,4X, 'RELRSD = ',E12.5)
C
C     *** TEST TO HALT ***
      JSTOP = MSSTOP(ISTOP,ITERS,ITMAX,ERRTOL,STPTST,IERROR,
