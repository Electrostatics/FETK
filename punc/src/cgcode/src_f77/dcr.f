      SUBROUTINE DCR(MATVEC,A,IA,X,B,N,IPARAM,RPARAM,IWORK,
     2   R,AR,AP,D,E,CNDWK,IERROR)
C***BEGIN PROLOGUE  DCR
...
C***LONG DEDCRIPTION
C
C    DCR IMPLEMENTS THE CONJUGATE RESIDUAL METHOD FOR SYMMETRIC 
...
C
C    DCR IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***ROUTINES CALLED  DONEST,D1MACH,DCGCHK,DAXPY,DCOPY,DDOT,DNRM2
C***END PROLOGUE  DCR
C
C     *** DECLARATIONS ***
...
C
C***FIRST EXECUTABLE STATEMENT  DCR
 1    CONTINUE
C
...
C     *** COMPUTE THE INITIAL RESIDUAL ***
 ...
 25   FORMAT(' INITIAL RESIDUAL = ', D12.5, /)
C
...
C     *** INITIALIZE P, AP ***
      ...
      RDOTAR = DDOT(N,R,1,AR,1)
      IF (RDOTAR .EQ. 0.0) THEN
         WRITE(IOUNIT,40) 
         GOTO 90
      END IF
      CALL DCOPY(N,R,1,B,1)
      CALL DCOPY(N,AR,1,AP,1)
C
C     *** UPDATE ITERS ***
 30   ITERS = ITERS + 1
C
C     *** COMPUTE NEW X ***
      ALPHA = RDOTAR / DNRM2(N,AP,1)**2
...
C     *** COMPUTE NEW P, AP ***
      ...
      IF (RDOTAR .EQ. 0.0) THEN
         WRITE(IOUNIT,40) 
 40      FORMAT(/, ' WARNING IN DCR: <R,AR> = 0', /)
         GOTO 90
      END IF
      BETA = RDOTAR/OLDRAR
      DO 45 I = 1, N
