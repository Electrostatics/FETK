      SUBROUTINE SCGNR(MATVEC,A,IA,X,B,N,IPARAM,RPARAM,IWORK,
     2   R,H,D,E,CNDWK,IERROR)
C***BEGIN PROLOGUE  SCGNR
...
C***LONG DESCRIPTION
C
C    SCGNR IMPLEMENTS THE CLASSICAL CONJUGATE GRADIENT METHOD APPLIED TO 
...
C
C    SCGNR IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE:
C
...
C
C***FIRST EXECUTABLE STATEMENT  SCGNR
 1    CONTINUE
C
...
C
C     *** COMPUTE NEW X ***
      ALPHA = (HNORM/SNRM2(N,H,1))**2
      CALL SAXPY(N,ALPHA,B,1,X,1)
C
C     *** COMPUTE AND CHECK NEW H ***
      CALL SAXPY(N,-ALPHA,H,1,R,1)
