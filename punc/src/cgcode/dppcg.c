/* ./src_f77/dppcg.f -- translated by f2c (version 20030320).
...
/* Subroutine */ int dppcg_(S_fp matvec, S_fp pcondl, doublereal *a, integer *
	ia, doublereal *x, doublereal *b, integer *n, doublereal *q, integer *
	iq, integer *iparam, doublereal *rparam, integer *iwork, doublereal *
	r__, doublereal *h__, doublereal *w1, doublereal *w2, doublereal *w3, 
	doublereal *w4, doublereal *d__, doublereal *e, integer *ierror)
{
    /* Format strings */
    static char fmt_5[] = "(\002 THE METHOD IS POLYNOMIAL PPCG\002,/)";
    static char fmt_10[] = "(\002 RESID  = DSQRT(<C(A)R,R>)\002,/,\002 RELRS"
	    "D = RESID / INITIAL RESID\002,/)";
    static char fmt_11[] = "(\002 RESID  = DSQRT(<C(C*A)C*R,R>)\002,/,\002 R"
	    "ELRSD = RESID / INITIAL RESID\002,/)";
    ...
    static doublereal alpha, erred, denom, ereps;
    static integer itmax;
    static doublereal sdumm;
    ...
    static doublereal s0norm;
    extern /* Subroutine */ int dcgchk_(integer *, doublereal *, integer *), 
	    ddpchb_(integer *, doublereal *, doublereal *, integer *, 
    ...
    static doublereal pegmin, pegmax, oldsnm;
...
/*    DPPCG IMPLEMENTS THE PRECONDITIONED CONJUGATE GRADIENT METHOD, */
...
/*    DPPCG IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE: */
...
/* ***FIRST EXECUTABLE STATEMENT  DPPCG */
...
/*     *** COMPUTE STOPPING CRITERION DENOMINATOR *** */
	...
	if (ipcond == 0) {
	    ...
	    denom = sqrt(ddot_(n, &b[1], &c__1, &h__[1], &c__1));
	} else {
...
/*     *** COMPUTE INITIAL H *** */
    ...
    s0norm = sqrt(ddot_(n, &r__[1], &c__1, &b[1], &c__1));
    snorm = s0norm;
    if (iounit > 0) {
...
/* Computing 2nd power */
    d__1 = snorm;
    alpha = d__1 * d__1 / ddot_(n, &h__[1], &c__1, &b[1], &c__1);
...
/*     *** COMPUTE NEW R AND H *** */
    ...
    oldsnm = snorm;
    snorm = sqrt(ddot_(n, &r__[1], &c__1, &h__[1], &c__1));
    if (iounit > 0) {
