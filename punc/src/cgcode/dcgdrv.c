/* ./src_f77/dcgdrv.f -- translated by f2c (version 20030320).
...
/* $Id: dcgdrv.c,v 1.3 2006/06/12 22:52:01 mholst Exp $ */
/* $Log: dcgdrv.c,v $
...
/* Changed the one instance of "FLOAT" to "DBLE" to make conversion to */
...
/* (2) calls to MAX were changed to calls to DMAX1 */
/* (3) calls to MIN were changed to calls to DMIN1 */
/* (4) calls to LOG were changed to calls to DLOG */

/* To change to double precision now simple involves: */

/* (1) change constants from:  x.yEz   to:   z.yDz */
/* (2) calls to DMAX1  will be changed to DMAX1 */
/* (3) calls to DMIN1  will be changed to DMIN1 */
/* (4) calls to DLOG   will be changed to DLOG */
/* (5) calls to D1MACH will be changed to D1MACH */
/* (6) calls to DNRM2  will be changed to DNRM2 */
/* (7) calls to DCOPY  will be changed to DCOPY */
/* (8) calls to DDOT   will be changed to DDOT */
/* (9) calls to DAXPY  will be changed to DAXPY */
/* (10)calls to DSCAL  will be changed to DSCAL */
/* (11)calls to DSQRT   will be changed to DDSQRT */
...
/* Add DCR into package, making for 10 cg methods. */
/* (Steve A. wants to provide both DCR and DCRIND; now done.) */
/* Documentation of package also updated to include DCR. */

/* Revision 1.12  90/02/19  10:27:48  holst */
/* Document/redo a little on MSTOP; DPPCG still needs work.  Others are */
/* ok I think.  See comments on last revision about DCRIND. */
...
/* (3) All stopping criterion implemented (I think) except for DPPCG. */
/*     (Should we add CONEST to DCRIND?  Should we add a SPCRIND?) */
/*     (There are other/better ways to implement MSTOP; look at?) */

/* Revision 1.10  90/02/14  21:34:12  holst */
/* Small fix to condition estimate code in DPPCG and DPCGCA */
...
/*     (a) uneccessary args not passed (PCONDL not passed to DCG, etc.) */
/*     (b) rwork parsed in DCGDRV so that the 9 subroutines are passed */
...
/* (3) DCGDRV conforms completely to the proposed standard. */
...
/*     access them to conform to DCGDRV requirements; eliminate having */
/*     to do translation. */
...
/* Bug fix in stopping criterion in routine DPCGNR. */
...
/*     (specifically DCRIND, DPPCG, and DPCGCA). */
...
/*     calls to MSTOP in 7 of the 9 cgcode routines;  DPPCG and DPCGCA */
/*     are buggy and a little difficult.  The next revision will deal */
/*     simply with implementing MSTOP for DPPCG and DPCGCA. */

/* Revision 1.3  90/02/12  03:16:24  holst */
/* This revision accomplished the following: */
/* (1) DCGDRV now conforms to the iterative standard; mcgdrv has been */
...
/* (2) All documentation of DCGDRV is complete.  All documentation of */
/*     the individual cgcode routines remains complete (see 3) */
/* (3) MSTOP has been implemented, and is used in DCG.  It appears */
