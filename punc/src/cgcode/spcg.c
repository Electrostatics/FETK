/* ./src_f77/spcg.f -- translated by f2c (version 20030320).
...
/* Subroutine */ int spcg_(S_fp matvec, S_fp pcondl, real *a, integer *ia, 
	real *x, real *b, integer *n, real *q, integer *iq, integer *iparam, 
	real *rparam, integer *iwork, real *r__, real *h__, real *d__, real *
	e, real *cndwk, integer *ierror)
{
    /* Format strings */
    static char fmt_6[] = "(\002 THE METHOD IS PRECONDITIONED CG (PCG)\002,/)"
	    ;
    static char fmt_8[] = "(4x,\002CNDMIA = \002,e12.5,/)";
    ...
    static char fmt_70[] = "(/,\002 NEW ESTIMATES FOR C*A:\002)";
    ...
    static real alpha, denom, oldrh, rdoth;
...
/*             ITERATIVE,PRECONDITION,CONJUGATE GRADIENTS */
...
/*             MAIL STOP B265 */
/*             LOS ALAMOS, NM 87545 */
/* ***PURPOSE  THIS ROUTINE SOLVES THE PRECONDITIONED SYMMETRIC POSITIVE */
/*            DEFINITE LINEAR SYSTEM C*AX = C*P.  BOTH A AND C */
/*            MUST BE SPD.  THE METHOD OF PRECONDITIONED CG IS USED. */
/* ***DESCRIPTION */
...
/*    SPCG IMPLEMENTS THE PRECONDITIONED CONJUGATE GRADIENT METHOD, */
...
/*    THIS ALGORITHM IS GUARANTEED TO CONVERGE IF BOTH A AND C ARE */
/*    SYMMETRIC POSITIVE DEFINITE.   MATHEMATICALLY, IF C*A HAS M */
...
/*    CODE STOPS WHEN COND(C*A)*(HNORM/H0NORM) .LE. ERRTOL, THEREBY */
/*    ATTEMPTING TO GUARANTEE THAT (FINAL RELATIVE ERROR) .LE. ERRTOL. */
/*    A NEW COND(C*A) ESTIMATE IS COMPUTED EVERY ICYCLE STEPS. THIS */
/*    IS DONE BY COMPUTING THE MIN AND MAX EIGENVALUES OF AN ORTHOGONAL */
/*    SECTION OF C*A. THE LARGEST ORTHOG SECTN HAS ORDER ICYCLE*NCE, */
...
/*    WHEN HNORM/H0NORM .LE. ERRTOL.  (ALSO SEE THE PROLOGUE TO SCGDRV.) */
...
/*    SPCG IS ONE ROUTINE IN A PACKAGE OF CG CODES; THE OTHERS ARE: */
...
/* ***ROUTINES CALLED  SONEST,R1MACH,SCGCHK,SAXPY,SDOT,SNRM2 */
/* ***END PROLOGUE  SPCG */

/*     *** DECLARATIONS *** */
/* CCCCCIMPLICIT  DOUBLE PRECISION(A-H,O-Z) */

/* ***FIRST EXECUTABLE STATEMENT  SPCG */
...
/*     *** CHECK THE INPUT PARAMETERS *** */
	...
	s_wsfe(&io___10);
	do_fio(&c__1, (char *)&cndmia, (ftnlen)sizeof(real));
	e_wsfe();
    }
    if (iounit > 0) {
...
/* L20: */
    ...
    (*pcondl)(&c__0, q, iq, &wdummy, &r__[1], &b[1], n);
    h0norm = snrm2_(n, &b[1], &c__1);
    if (iounit > 0) {
	io___21.ciunit = iounit;
	s_wsfe(&io___21);
	do_fio(&c__1, (char *)&h0norm, (ftnlen)sizeof(real));
	e_wsfe();
    }
...
/*     *** INITIALIZE RDOTH *** */
    rdoth = sdot_(n, &r__[1], &c__1, &b[1], &c__1);
...
/*     *** COMPUTE NEW X *** */
    alpha = rdoth / sdot_(n, &h__[1], &c__1, &b[1], &c__1);
...
/*     *** COMPUTE AND CHECK NEW H *** */
	...
	do_fio(&c__1, (char *)&hnorm, (ftnlen)sizeof(real));
	r__1 = hnorm / h0norm;
...
/*     *** COMPUTE NEW P  *** */
    oldrh = rdoth;
    rdoth = sdot_(n, &r__[1], &c__1, &h__[1], &c__1);
    beta = rdoth / oldrh;
    i__1 = *n;
    for (i__ = 1; i__ <= i__1; ++i__) {
...
/*        *** UPDATE PARAMETERS *** */
	...
	if (iters % icycle == 0) {
	    ...
	    sonest_(&iounit, &d__[1], &e[1], &cndwk[1], &cndwk[kmax + 1], &
		    iwork[1], &iters, &eigmin, &eigmax, &cndmia);
	}
    }

/*     *** RESUME PCG ITERATION *** */
    goto L30;
